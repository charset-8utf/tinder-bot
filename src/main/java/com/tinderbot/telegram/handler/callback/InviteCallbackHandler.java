...
public class InviteCallbackHandler implements CallbackHandler {
    ...
    public boolean supports(String callback) {
        return CallbackConstants.INVITE.equals(callback);
    }

    @Override
    public void execute(MultiSessionTelegramBot bot, Long chatId, String callback) {
        TextGenerationResult result = messageDialogService.generateDateInvite(chatId);
        if (result.hasEmptyHistory()) {
            bot.sendTextMessage("Сначала отправьте вашу переписку.");
            return;
        }

        Message confirmMsg = bot.sendTextMessage("✅ Ваше сообщение принято!");
        telegramUi.addBotMessageId(chatId, confirmMsg.getMessageId());

        Message thinkingMsg = bot.sendTextMessage("ChatGPT придумывает приглашение...");
